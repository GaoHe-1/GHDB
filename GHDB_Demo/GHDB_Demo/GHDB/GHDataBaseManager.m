...
//  GHDB
//
//  Created by Rafael on 16/11/8.
//  Copyright © 2016年 GaoHe. All rights reserved.
//

#import "GHDataBaseManager.h"
#import "GHDataBaseManager+OperationToolsMethod.h"

#pragma mark -  创建数据库
@implementation GHDataBaseManager
...
/** 开启数据库 */
+ (void)openDB {
    [GHDataBaseManager shareDataBase];
}

/** 关闭数据库 */
+ (void)closeDB {
    sqlite3 *db = [GHDataBaseManager shareDataBase];
...
- (sqlite3 *)db {
    if (!_db) {
        self.db = [GHDataBaseManager shareDataBase];
    }
    return _db;
}
...
#pragma mark -  数据库初始化
@implementation GHDataBaseManager (CreateDataBaseTable)
...
#pragma mark -  操作数据库
@implementation GHDataBaseManager (OperationSQLiteTable)
/** 向表中添加model */
